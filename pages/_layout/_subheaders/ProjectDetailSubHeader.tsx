...
const ProjectDetailSubHeader = () => {
  ...
  const { setMonitorUrl, setDbMonitorUrl, projectData, isbuildActiveStatus, setIsbuildActiveStatus } =
    useProjectContext();
  ...
  if (navUrl) {
    ...
    setDbMonitorUrl(navUrl.db_monitor_url);
  }
  ...
  const buttonsData = ['HISTORY', 'SHELL', 'EDITOR', 'MONITOR', 'LOGS', 'BACKUPS', 'UPGRADE', 'SETTINGS'];
        ...
        <SubHeaderLeft>
          {buttonsData.map((buttonText, index) => (
            <CustomButton
              ...
              disabled={
                (!isbuildActiveStatus &&
                  (buttonText === 'SHELL' || buttonText === 'EDITOR' || buttonText === 'MONITOR')) ||
                (stage === 'Development' && buttonText === 'BACKUPS') ||
                ((navUrl?.shell_url === '' || navUrl?.shell_url === null) && buttonText === 'SHELL') ||
                ((navUrl?.editor_url === '' || navUrl?.editor_url === null) && buttonText === 'EDITOR') ||
                ((navUrl?.monitor_url === '' || navUrl?.monitor_url === null) && buttonText === 'MONITOR')
              }
            />
          ))}
        </SubHeaderLeft>
