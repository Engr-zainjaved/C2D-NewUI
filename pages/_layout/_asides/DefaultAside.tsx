...
const DefaultAside = () => {
  ...
  useEffect(() => {
    if (webSocketData) {
      ...
      if (webSocketData.project_id == selectedProjectId) {
      ...
      if (eventType === 'download_ready' && webSocketData.project_id == selectedProjectId) {
              ...
              <React.Fragment key={environment}>
                {environment !== 'Production' && (
                  <EnvironmentDropHandler
                    ...
                    onDrop={(branchId: number) => stageChange(branchId, branches[environment as keyof Branches].id)}
                    formik={formik}
