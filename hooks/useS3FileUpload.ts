...
const useS3FileUpload = () => {
  ...
  async function uploadFileToS3(filename: string, file: any, projectId: number, branchId: number, importType: string) {
    ...
    try {
      ...
      const hostedUrl = presignedUrl;
      importDatabase(projectId, branchId, importType, hostedUrl, filename, s3Key);
    } catch (error) {
