...
export function isBuildActive(arr: any) {
  ...
  if (arr[0]?.build?.status == 'in_progress' && arr.length > 1 && arr[1]?.build?.status === 'success') {
    return true;
  }
...
export function updateStatus(apiData: any, websocketData: any, selectedBranchId: string) {
  ...
  if (updatedData) {
    if (websocketData?.type === 'branch_updated' && selectedBranchId == websocketData?.data?.branch.id) {
      updatedData.unshift(websocketData.data.tracking);
      return updatedData;
    } else if (websocketData?.type === 'branch_updated' && selectedBranchId == websocketData.branch_id) {
      updatedData.unshift(websocketData.tracking);
      return updatedData;
    } else if (
