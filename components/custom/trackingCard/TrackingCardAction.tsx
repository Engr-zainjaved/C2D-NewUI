...
const TrackingCardAction = ({ status, buildId, errorMessage, buildUrl }: TrackingCardActionProps) => {
  useEffect(() => {
      ...
      <span style={{ display: 'flex', alignItems: 'center' }}>
        ...
        {status === 'success' && buildUrl ? (
          <Dropdown isButtonGroup>
            <Button color="success" onClick={() => window.open(buildUrl, '_blank')} style={{ marginLeft: '10px' }}>
              Connect
            </Button>
            <DropdownToggle>
